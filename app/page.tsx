...
const translations = {
  ro: {
    ...
    hero: {
      eyebrow: "Creat pentru afaceri în creștere",
      titleStart: "Angajează primul tău",
      titleHighlight: "angajat AI.",
      description:
        "DavidPilot răspunde clienților, califică lead-uri și programează întâlniri, astfel încât echipa ta să se poată concentra pe dezvoltarea afacerii.",
      primaryButton: "Programează un demo gratuit",
      secondaryButton: "Testează DavidPilot",
      availability: "Disponibil 24/7",
      setup: "Configurare în câteva zile",
      integrations: "Funcționează cu instrumentele tale",
    },
    demo: {
      available: "Disponibil 24/7",
      greeting:
  ...
  en: {
    ...
    hero: {
      ...
      description:
        "DavidPilot answers customers, qualifies leads and books appointments—so your team can focus on growing the business.",
      primaryButton: "Book a free demo",
      secondaryButton: "Try DavidPilot",
...
function Demo({ language }: { language: Language }) {
  ...
  const lead = useMemo(
    () => ({
      business: step >= 2 ? t.serviceCompany : "—",
      inquiries: step >= 3 ? "80+" : "—",
      score: step >= 3 ? "92/100" : t.building,
      saved: step >= 3 ? "15 hrs/week" : t.calculating,
    }),
    [step, t]
  );
  ...
  const send = (preset?: string) => {
    ...
    window.setTimeout(() => {
      setMessages((current) => [
        ...
        {
          sender: "ai",
          text: t.replies[Math.min(step, t.replies.length - 1)],
        },
      ]);
      setStep(nextStep);
      setTyping(false);

...
const t = {
  en: {
    ...
    founderTitle:"Leadership grounded in engineering.", founderText:"Founded by Gabriel Dobrescu, an engineering leader with more than a decade of experience across software engineering, cloud, DevOps, enterprise platforms and technical leadership.", founderLink:"Meet the founder",
  ...
  ro: {
    ...
    founderTitle:"Leadership construit pe experiență tehnică.", founderText:"Fondat de Gabriel Dobrescu, lider în inginerie cu peste un deceniu de experiență în software, cloud, DevOps, platforme enterprise și coordonare tehnică.", founderLink:"Descoperă fondatorul",
...
function RoiCalculator({ lang }: { lang: Lang }) {
  ...
  return <section className="roi-section" id="roi"><div className="container roi-grid">
    <div className="roi-copy"><span className="eyebrow">{c.roiEyebrow}</span><h2>{c.roiTitle}</h2><p>{c.roiIntro}</p><Link href="/contact" className="button button-light">{c.roiCta}</Link></div>
...
export default function Home() { return <PageFrame>{({ lang }) => { const c = t[lang]; return <>
  <section className="hero container"><div className="hero-copy"><span className="eyebrow">{c.eyebrow}</span><h1>{c.title}</h1><p>{c.intro}</p><div className="hero-actions"><Link className="button" href="/contact">{c.primary}</Link><a className="button button-ghost" href="#solutions">{c.secondary}</a></div></div><div className="system-visual" aria-hidden="true"><div className="system-grid"/><div className="system-core"><small>DAVIDPILOT</small><strong>AI OPERATING LAYER</strong><span>Secure • Integrated • Observable</span></div><div className="orbit orbit-one"/><div className="orbit orbit-two"/></div></section>

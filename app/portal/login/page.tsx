...
export default function PortalLogin(){
 ...
 async function submit(e:React.FormEvent){e.preventDefault();setLoading(true);setError("");const r=await fetch("/api/portal/login",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({email,password})});const data=await r.json().catch(()=>({}));setLoading(false);if(!r.ok){setError(data.error||"Could not sign in.");return}router.push("/portal");router.refresh()}
 return <main className="portal-login"><section className="portal-login-card"><div className="portal-brand"><span>DP</span><div><strong>DAVIDPILOT</strong><small>Client Portal</small></div></div><span className="portal-kicker">SECURE CLIENT WORKSPACE</span><h1>Welcome back.</h1><p>Access projects, meetings, proposals, documents and support in one private workspace.</p><form onSubmit={submit}><label>Email<input type="email" value={email} onChange={e=>setEmail(e.target.value)} autoComplete="email" required/></label><label>Password<input type="password" value={password} onChange={e=>setPassword(e.target.value)} autoComplete="current-password" required/></label>{error&&<div className="portal-error">{error}</div>}<button disabled={loading}>{loading?"Signing in…":"Sign in securely"}</button></form><div className="portal-login-links"><a href="/portal/forgot-password">Forgot password?</a></div><small className="portal-help">Need access? Contact your DavidPilot project lead.</small></section></main>
}

...
export async function GET() {
  ...
  try {
    ...
    const [projects, milestones, meetings, documents, proposals, support, activity] = await Promise.all([
    ...
    return NextResponse.json({ user: { id: user.id, email: user.email }, client: membership.clients, role: membership.role, projects, milestones, meetings, documents, proposals, support, activity });
  } catch (error) {

...
const copy = {
  en: {
    ...
    leadTitle: "Save your details before booking",
    leadIntro: "We will attach your conversation to the enquiry so the strategy session starts with context.",
    name: "Name", company: "Company", email: "Business email", phone: "Phone",
    saveLead: "Save details & continue", savingLead: "Saving…",
    leadSuccess: "Your enquiry has been saved. You can now choose a meeting time.",
    leadError: "We could not save your details. Please check the fields and try again.",
  },
  ro: {
    nav: [
      ["Soluții", "/solutions"],
    ...
    leadTitle: "Salvează datele înainte de programare",
    leadIntro: "Vom atașa conversația cererii, astfel încât sesiunea strategică să înceapă cu contextul potrivit.",
    name: "Nume", company: "Companie", email: "Email de business", phone: "Telefon",
    saveLead: "Salvează datele și continuă", savingLead: "Se salvează…",
    leadSuccess: "Solicitarea a fost salvată. Acum poți alege intervalul pentru întâlnire.",
    leadError: "Datele nu au putut fi salvate. Verifică informațiile și încearcă din nou.",
  },
};
...
function BookingModal({ lang }: { lang: Lang }) {
  ...
  const calLinkEn = (process.env.NEXT_PUBLIC_CAL_LINK_EN || process.env.NEXT_PUBLIC_CAL_LINK || "").trim();
  const calLinkRo = (process.env.NEXT_PUBLIC_CAL_LINK_RO || process.env.NEXT_PUBLIC_CAL_LINK || "").trim();
  const calLink = lang === "ro" ? calLinkRo : calLinkEn;
  ...
  const text = lang === "ro"
    ? {
        ...
        missingHelp: "Adaugă NEXT_PUBLIC_CAL_LINK_RO în Vercel, folosind linkul evenimentului Cal.com în limba română.",
    ...
    : {
        ...
        missingHelp: "Add NEXT_PUBLIC_CAL_LINK_EN in Vercel using your English Cal.com event link.",
        email: "Send an email",
      };
      ...
      <section className="booking-modal">
        ...
        <div className="booking-calendar">
          {normalized ? (
            <iframe
              src={normalized}
              title={lang === "ro" ? "Programare sesiune strategică DavidPilot" : "DavidPilot strategy session booking"}
              loading="eager"
...
export function ChatConsultant({ lang }: { lang: Lang }) {
  ...
  useEffect(() => {
    ...
    const savedContact = window.localStorage.getItem("davidpilot-chat-contact");
    const savedLeadStatus = window.localStorage.getItem("davidpilot-chat-lead-saved");
    if (savedMessages) {
    ...
    if (savedContact) {
      try { setContact(JSON.parse(savedContact)); } catch { /* ignore invalid local data */ }
    }
    setLeadSaved(savedLeadStatus === "true");
  }, []);
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    window.localStorage.setItem("davidpilot-chat-contact", JSON.stringify(contact));
    window.localStorage.setItem("davidpilot-chat-lead-saved", String(leadSaved));
  }, [messages, lead, contact, leadSaved]);
  useEffect(() => {
  ...
  async function submitLead(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setLeadSaved(true);
      setMessages((current) => [...current, { sender: "ai", text: copy[lang].leadSuccess }]);
    } catch {
      setLeadFormError(copy[lang].leadError);
    } finally {
      setLeadSaving(false);
    }
  }
  ...
  return (
    <>
      ...
      {open && <section className="chat-panel chat-panel-visible" aria-label={copy[lang].chatTitle}>
        ...
        {lead.qualified && !leadSaved && (
          <form className="chat-lead-form" onSubmit={submitLead}>
            <div className="chat-lead-copy">
              <strong>{copy[lang].leadTitle}</strong>
              <span>{copy[lang].leadIntro}</span>
            </div>
            <div className="chat-lead-grid">
              <input required minLength={2} value={contact.name} onChange={(e) => setContact((current) => ({ ...current, name: e.target.value }))} placeholder={copy[lang].name} />
              <input required minLength={2} value={contact.company} onChange={(e) => setContact((current) => ({ ...current, company: e.target.value }))} placeholder={copy[lang].company} />
              <input required type="email" value={contact.email} onChange={(e) => setContact((current) => ({ ...current, email: e.target.value }))} placeholder={copy[lang].email} />
              <input required minLength={6} value={contact.phone} onChange={(e) => setContact((current) => ({ ...current, phone: e.target.value }))} placeholder={copy[lang].phone} />
            </div>
            {leadFormError && <p className="chat-lead-error">{leadFormError}</p>}
            <button type="submit" disabled={leadSaving}>{leadSaving ? copy[lang].savingLead : copy[lang].saveLead}</button>
          </form>
        )}
        {lead.qualified && leadSaved && <BookingButton className="chat-contact-link" onClick={() => setOpen(false)}>{copy[lang].contact} →</BookingButton>}

  ...
  return (
    <>
      {open && <section className="chat-panel" aria-label={copy[lang].chatTitle}>
        <div className="chat-head"><div><span className="status-dot" />{copy[lang].chatTitle}</div><div className="chat-head-actions"><button className="chat-reset" onClick={resetConversation}>{copy[lang].reset}</button><button onClick={() => setOpen(false)} aria-label="Close">×</button></div></div>
      ...
      <button className={`chat-launcher ${open ? "open" : ""}`} onClick={() => setOpen((v) => !v)} aria-label="Open AI consultant"><span>{open ? "×" : "AI"}</span>{!open && <strong>{copy[lang].chatLauncher}</strong>}</button>
    </>
  );
}
